...
export default function Home() {
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      const index = Math.max(0, projects.findIndex(project => project.id === selectedProject))
      if (event.key === 'ArrowRight') setSelectedProject(projects[(index + 1) % projects.length].id)
      if (event.key === 'ArrowLeft') setSelectedProject(projects[(index - 1 + projects.length) % projects.length].id)
    }
