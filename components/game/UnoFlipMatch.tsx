...
import { colorMapping, faceMapping, initGame, isFlipAvailable, unoFlipReducer, type CardColor, type UnoFlipState } from '@/games/uno-flip'

export interface UnoFlipMatchProps { readonly onClose: () => void; readonly seed?: number }
const STORAGE_KEY = 'khadijat-uno-flip-match-v3'
  ...
  return <motion.div className="fixed inset-0 z-[80] grid place-items-center bg-[#111217]/85 p-4 backdrop-blur-md" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} role="dialog" aria-modal="true" aria-labelledby="uno-title">
    ...
    <motion.section className="open-book relative grid max-h-[92vh] min-h-[650px] w-full max-w-6xl grid-cols-1 overflow-y-auto rounded-md bg-[#FAF8F4] shadow-2xl md:grid-cols-[1.25fr_.75fr]" initial={{ scale: .9, y: 24 }} animate={{ scale: 1, y: 0 }} exit={{ scale: .94, y: 20 }}>
      ...
      <aside className="flex flex-col bg-black/[.035] p-8 pt-20 md:p-10 md:pt-24">
        ...
        <dl className="mt-8 grid grid-cols-3 gap-3 border-y border-black/15 py-5"><div><dt className="font-mono text-[8px] uppercase">Moves</dt><dd className="font-serif text-3xl">{state.moves}</dd></div><div><dt className="font-mono text-[8px] uppercase">Pairs</dt><dd className="font-serif text-3xl">{state.matchedPairIds.length}/{state.grid.pairCount}</dd></div><div><dt className="font-mono text-[8px] uppercase">Side</dt><dd className="mt-2 font-mono text-xs uppercase">{state.grid.side}</dd></div></dl>
        <button type="button" onClick={() => dispatch({ type: 'select', cardId: state.grid.flipCard.id })} className="mt-7 rounded-xl border-2 border-[#7250ad] p-5 text-left text-white" style={{ backgroundColor: palette[state.grid.flipCard.color] }}><p className="font-mono text-xs font-bold">{state.grid.flipCard.icon} FLIP</p><p className="mt-3 text-sm leading-6">{isFlipAvailable(state) ? 'Control card — not part of the memory grid. Activate it to turn every face over.' : 'FLIP unavailable.'}</p></button>
