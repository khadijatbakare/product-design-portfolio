...
import { darkToLight, initGame, isFlipAvailable, lightToDark, unoFlipReducer, type CardColor, type UnoFlipState } from '@/games/uno-flip'

export interface UnoFlipMatchProps { readonly onClose: () => void; readonly seed?: number }
const STORAGE_KEY = 'khadijat-uno-flip-match-v1'
const palette: Record<UnoFlipState['side'], Record<CardColor, string>> = { light: { blue: '#2f63d7', red: '#d7463f', green: '#3c8f62', yellow: '#e2b93f', flip: '#7250ad', wild: '#25252a' }, dark: { blue: '#00a7a0', red: '#f20f74', green: '#9bc400', yellow: '#ff711a', flip: '#b031dc', wild: '#060608' } }
    ...
    <motion.section className="open-book relative grid max-h-[92vh] min-h-[650px] w-full max-w-6xl grid-cols-1 overflow-y-auto rounded-md bg-[#FAF8F4] shadow-2xl md:grid-cols-[1.25fr_.75fr]" initial={{ scale: .9, y: 24 }} animate={{ scale: 1, y: 0 }} exit={{ scale: .94, y: 20 }}>
      ...
      <main className="flex flex-col justify-center p-6 pt-20 md:p-10 md:pt-24">
        <AnimatePresence mode="wait"><motion.div key={`${state.side}-${state.flipCount}`} className="grid grid-cols-3 gap-3 sm:grid-cols-4" initial={{ rotateY: -90, filter: 'brightness(2) saturate(1.8)' }} animate={{ rotateY: 0, filter: 'brightness(1) saturate(1)' }} exit={{ rotateY: 90, filter: 'brightness(2) saturate(1.8)' }} transition={{ duration: .5 }} style={{ perspective: 1000 }}>
          {state.grid.cards.map((card) => { const faceUp = selected.has(card.id) || matched.has(card.pairId); const face = card[state.side]; const resolves = state.phase === 'resolving' && state.selectedIds[1] === card.id; return <motion.button key={card.id} type="button" onClick={() => dispatch({ type: 'select', cardId: card.id })} onAnimationComplete={() => { if (resolves) dispatch({ type: 'resolve' }) }} className="relative aspect-[.7] rounded-xl focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2" animate={{ rotateY: faceUp ? 180 : 0, opacity: matched.has(card.pairId) ? .4 : 1 }} transition={{ duration: .3 }} style={{ transformStyle: 'preserve-3d' }} aria-label={faceUp ? `${face.display}, face up` : 'Face-down card'}><span className="absolute inset-0 grid place-items-center rounded-xl border-4 border-white bg-[#202126] font-mono text-2xl text-white shadow-md [backface-visibility:hidden]">UNO</span><span className="absolute inset-0 grid place-items-center rounded-xl border-4 border-white p-2 text-center font-mono text-sm font-bold text-white shadow-md [backface-visibility:hidden]" style={{ backgroundColor: palette[state.side][face.color], transform: 'rotateY(180deg)' }}>{face.display}</span></motion.button> })}
      ...
      <aside className="flex flex-col bg-black/[.035] p-8 pt-20 md:p-10 md:pt-24">
        ...
        <dl className="mt-8 grid grid-cols-3 gap-3 border-y border-black/15 py-5"><div><dt className="font-mono text-[8px] uppercase">Moves</dt><dd className="font-serif text-3xl">{state.moves}</dd></div><div><dt className="font-mono text-[8px] uppercase">Pairs</dt><dd className="font-serif text-3xl">{state.matchedPairIds.length}/6</dd></div><div><dt className="font-mono text-[8px] uppercase">Side</dt><dd className="mt-2 font-mono text-xs uppercase">{state.side}</dd></div></dl>
        <section className="mt-7 rounded-xl border-2 border-[#7250ad] bg-[#7250ad] p-5 text-white"><p className="font-mono text-xs font-bold">FLIP</p><p className="mt-3 text-sm leading-6">{isFlipAvailable(state) ? 'Match this pair to turn every card over.' : 'Activated — the dark side is in play.'}</p></section>
        <section className="mt-7"><p className="font-mono text-[8px] uppercase tracking-widest">Light ↔ Dark</p><div className="mt-3 space-y-2">{(['blue','red','green','yellow','wild'] as const).map((color) => <div key={color} className="flex justify-between border-b border-dashed border-black/15 pb-1 font-mono text-[9px]"><span>{darkToLight[color].display}</span><span>→</span><span>{lightToDark[color].display}</span></div>)}</div></section>
