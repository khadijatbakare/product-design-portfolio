  ...
  return <motion.div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4 backdrop-blur-md" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} role="dialog" aria-modal="true" aria-labelledby="modal-title">
    <motion.article className="open-book relative grid max-h-[92vh] min-h-[620px] w-full max-w-6xl grid-cols-1 overflow-y-auto rounded-md shadow-2xl md:grid-cols-2" initial={{ scale: .92, y: 28 }} animate={{ scale: 1, y: 0 }} exit={{ scale: .95, y: 20 }} transition={{ type: 'spring', stiffness: 260, damping: 28 }}>
      ...
      {activeModal === 'work' ? <><WorkArgument project={project}/><WorkEvidence project={project} onSelectProject={onSelectProject}/></> : <>
        <div className="p-10 md:p-16"><Label>{activeModal === 'resume' ? aboutMe.kicker : `${activeModal} / Library volume`}</Label><h2 id="modal-title" className="mt-10 font-serif text-5xl leading-none md:text-7xl">{activeModal === 'resume' ? aboutMe.headline.map((line) => <span key={line} className={`block ${line === aboutMe.emphasis ? 'italic' : ''}`}>{line}</span>) : volume?.heading ?? 'Let’s make sense of something together.'}</h2>{volume && <p className="mt-8 max-w-md text-lg leading-8 text-black/65">{activeModal === 'resume' ? aboutMe.intro : volume.description}</p>}{activeModal === 'resume' && <div className="mt-10 space-y-5 text-base leading-7">{aboutMe.story.map((paragraph) => <p key={paragraph} className={paragraph.startsWith('[TODO') ? 'border border-dashed border-black/30 bg-amber-50/60 p-4 font-mono text-xs leading-6' : ''}>{paragraph}</p>)}<section className="border-t border-black/15 pt-6"><Label>Education</Label>{resume.education.map((entry) => <div key={entry.id} className="mt-3"><p className="font-medium">{entry.qualification}</p><p className="text-sm text-black/55">{entry.school} · {entry.period}</p></div>)}</section></div>}{activeModal === 'contact' && <p className="mt-10 max-w-sm text-lg leading-8">{checkoutSlip.availability.label}. If there is a thoughtful product problem to untangle, I’d like to hear about it.</p>}</div>
        <div className="flex flex-col justify-center bg-white/35 p-10 md:p-16">{activeModal === 'notes' && <div className="grid auto-rows-min gap-5 sm:grid-cols-2">{aboutMe.hobbies.map((hobby, index) => <article key={hobby.id} className={`${hobbySpan[hobby.span]} ${index % 3 === 0 ? '-rotate-1' : 'rotate-1'} relative flex flex-col justify-end overflow-hidden border border-black/15 p-5 shadow-sm`} style={{ backgroundColor: hobby.media.placeholder }} aria-label={hobby.media.alt}><div className="absolute inset-0 bg-gradient-to-t from-black/55 via-transparent to-white/10"/><div className="relative text-white"><Label>0{index + 1} / {hobby.label}</Label><h3 className="mt-4 font-serif text-3xl">{hobby.headline}</h3><p className="mt-3 text-sm leading-6 text-white/85">{hobby.caption}</p></div></article>)}</div>}{activeModal === 'resume' && <><div><Label>{resume.label}</Label><h3 className="mt-3 font-serif text-4xl">{resume.headline}</h3><p className="mt-4 leading-7 text-black/65">{resume.summary}</p></div><div className="mt-10 space-y-8">{resume.experience.map((entry) => <article key={entry.id}><Label>{entry.period} · {entry.location}</Label><h3 className="mt-2 font-serif text-3xl">{entry.role}</h3><p className="mt-1 text-sm">{entry.company}</p><p className="mt-4 leading-7">{entry.summary}</p><ul className="mt-3 list-disc space-y-1 pl-5 text-sm">{entry.highlights.map((highlight) => <li key={highlight}>{highlight}</li>)}</ul></article>)}</div><div className="mt-10 border-t border-black/15 pt-8"><Label>Core skills</Label><p className="mt-3 text-sm leading-6">{resume.coreSkills.join(' · ')}</p></div><div className="mt-8"><Label>Design-system depth</Label><div className="mt-4 space-y-4">{resume.designSystemSkills.map((skill) => <article key={skill.id} className="grid grid-cols-[1fr_auto] gap-x-4"><p className="font-medium">{skill.label}</p><span className="font-mono text-[8px] uppercase tracking-widest">{skill.proficiency}</span><p className="col-span-2 mt-1 text-sm leading-6 text-black/55">{skill.note}</p></article>)}</div></div><div className="mt-8"><Label>Tools</Label><p className="mt-3 text-sm leading-6">{resume.tools.join(' · ')}</p></div><button onClick={() => window.print()} className="mt-10 flex w-fit items-center gap-2 rounded-full bg-ink px-5 py-3 text-sm text-paper"><Download size={16}/> {resume.downloadLabel}</button></>}{activeModal === 'contact' && <><a href={`mailto:${siteIdentity.email}`} className="flex items-center gap-3 text-xl underline underline-offset-8"><Mail/> {siteIdentity.email}</a><dl className="mt-10 space-y-4">{checkoutSlip.ledger.map((entry) => <div key={entry.label} className="flex justify-between gap-6 border-b border-dashed border-black/20 pb-2"><dt><Label>{entry.label}</Label></dt><dd>{entry.value}</dd></div>)}</dl></>}</div>
      </>}
    </motion.article>
  </motion.div>
}
