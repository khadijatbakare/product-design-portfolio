...
import { aboutMe, checkoutSlip, getProjectsByVolume, libraryCopy, projects, resume, volumes, type ModalView, type Project } from '@/data/content'
  ...
  return <motion.div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4 backdrop-blur-md" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} role="dialog" aria-modal="true" aria-labelledby="modal-title">
    <motion.article className="open-book relative grid max-h-[92vh] min-h-[620px] w-full max-w-6xl grid-cols-1 overflow-y-auto rounded-md shadow-2xl md:grid-cols-2" initial={{ scale: .92, y: 28 }} animate={{ scale: 1, y: 0 }} exit={{ scale: .95, y: 20 }} transition={{ type: 'spring', stiffness: 260, damping: 28 }}>
      ...
      {activeModal === 'work' ? <><WorkArgument project={project}/><WorkEvidence project={project} onSelectProject={onSelectProject}/></> : <>
        <div className="p-10 md:p-16"><Label>{activeModal} / Library volume</Label><h2 id="modal-title" className="mt-10 font-serif text-5xl leading-none md:text-7xl">{volume?.heading ?? 'Let’s make sense of something together.'}</h2>{volume && <p className="mt-8 max-w-md text-lg leading-8 text-black/65">{volume.description}</p>}{activeModal === 'resume' && <div className="mt-10 space-y-5 text-base leading-7">{aboutMe.story.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}<p className="font-mono text-xs uppercase tracking-widest">{resume.education}</p></div>}{activeModal === 'contact' && <p className="mt-10 max-w-sm text-lg leading-8">{checkoutSlip.availability.label}. If there is a thoughtful product problem to untangle, I’d like to hear about it.</p>}</div>
        <div className="flex flex-col justify-center bg-white/35 p-10 md:p-16">{activeModal === 'notes' && <div className="grid gap-5 sm:grid-cols-2">{aboutMe.hobbies.map((hobby, index) => <article key={hobby.id} className={`${index % 3 === 0 ? '-rotate-1' : 'rotate-1'} border border-black/15 bg-[#f8f1e5] p-5 shadow-sm`}><Label>0{index + 1} / {hobby.title}</Label><p className="mt-5 font-serif text-2xl">{hobby.caption}</p></article>)}</div>}{activeModal === 'resume' && <><div className="space-y-8">{resume.entries.map((entry) => <article key={`${entry.organization}-${entry.period}`}><Label>{entry.period}</Label><h3 className="mt-2 font-serif text-3xl">{entry.role}</h3><p className="mt-1 text-sm">{entry.organization}</p><p className="mt-4 leading-7">{entry.summary}</p><ul className="mt-3 list-disc space-y-1 pl-5 text-sm">{entry.highlights.map((highlight) => <li key={highlight}>{highlight}</li>)}</ul></article>)}</div><div className="mt-10 grid gap-6 border-t border-black/15 pt-8 sm:grid-cols-2"><div><Label>Core skills</Label><p className="mt-3 text-sm leading-6">{resume.coreSkills.join(' · ')}</p></div><div><Label>Design systems</Label><p className="mt-3 text-sm leading-6">{resume.designSystemSkills.join(' · ')}</p></div></div><button onClick={() => window.print()} className="mt-10 flex w-fit items-center gap-2 rounded-full bg-ink px-5 py-3 text-sm text-paper"><Download size={16}/> Download résumé</button></>}{activeModal === 'contact' && <><a href={`mailto:${aboutMe.email}`} className="flex items-center gap-3 text-xl underline underline-offset-8"><Mail/> {aboutMe.email}</a><dl className="mt-10 space-y-4">{checkoutSlip.ledger.map((entry) => <div key={entry.label} className="flex justify-between gap-6 border-b border-dashed border-black/20 pb-2"><dt><Label>{entry.label}</Label></dt><dd>{entry.value}</dd></div>)}</dl></>}</div>
      </>}
    </motion.article>
  </motion.div>
}
