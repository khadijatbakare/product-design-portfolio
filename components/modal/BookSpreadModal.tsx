...
import { aboutMe, checkoutSlip, getProjectsByVolume, libraryCopy, projects, resume, volumes, type ModalView } from '@/data/content'
...
export function BookSpreadModal({ activeModal, selectedProject, onClose, onSelectProject }: Props) {
  ...
  const volume = activeModal === 'work'
    ? volumes.find((item) => item.id === project.volumeId)
    : volumes.find((item) => item.contents === activeModal)

  const title = volume?.heading ?? 'Let’s make sense of something together.'
  ...
  return (
    <motion.div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4 backdrop-blur-md" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} role="dialog" aria-modal="true" aria-labelledby="modal-title">
      <motion.article className="open-book relative grid max-h-[90vh] min-h-[620px] w-full max-w-6xl grid-cols-1 overflow-y-auto rounded-md shadow-2xl md:grid-cols-2" initial={{ scale: .92, y: 28 }} animate={{ scale: 1, y: 0 }} exit={{ scale: .95, y: 20 }} transition={{ type: 'spring', stiffness: 260, damping: 28 }}>
        <button autoFocus onClick={onClose} className="absolute right-5 top-5 z-10 flex items-center gap-2 rounded-full border border-black/20 bg-white/70 px-4 py-2 font-mono text-[9px] uppercase tracking-widest" aria-label={libraryCopy.back}><X size={15}/>{libraryCopy.back}</button>
        <div className="p-10 md:p-16">
          ...
          {volume && <p className="mt-8 max-w-md text-lg leading-8 text-black/65">{volume.description}</p>}
          {activeModal === 'work' && <p className="mt-8 font-mono text-[10px] uppercase tracking-widest">{project.discipline}</p>}
          {activeModal === 'resume' && <div className="mt-10 space-y-5 text-base leading-7">{aboutMe.story.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}<p className="font-mono text-xs uppercase tracking-widest">{resume.education}</p></div>}
        ...
        <div className="flex flex-col justify-center bg-white/35 p-10 md:p-16">
          {activeModal === 'work' && <>
            <h3 className="mb-10 font-serif text-4xl leading-tight">{project.title}</h3>
          ...
          {activeModal === 'notes' && <div className="grid gap-5 sm:grid-cols-2">{aboutMe.hobbies.map((hobby, index) => <article key={hobby.id} className={`${index % 3 === 0 ? '-rotate-1' : 'rotate-1'} border border-black/15 bg-[#f8f1e5] p-5 shadow-sm`}><span className="font-mono text-[9px] uppercase tracking-widest">0{index + 1} / {hobby.title}</span><p className="mt-5 font-serif text-2xl">{hobby.caption}</p></article>)}</div>}
          {activeModal === 'resume' && <>
