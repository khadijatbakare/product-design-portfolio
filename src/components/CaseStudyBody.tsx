...
const label = (beat: Beat, number: string) => `${number} / ${beatLabels[beat]}`
function assertNever(value: never): never { throw new Error(`Unhandled case study block: ${JSON.stringify(value)}`) }

export function ProseBlock({ block, number }: { block: BlockOf<'prose'>; number: string }) {
  return <section className="case-section section-wrap narrow"><div className="case-label">{label(block.beat, number)}</div><div><h2>{block.heading}</h2>{block.paragraphs.map(paragraph => <p key={paragraph}>{paragraph}</p>)}</div></section>
}
...
export function InsightBlock({ block, number }: { block: BlockOf<'insight'>; number: string }) {
  return <section className="case-section section-wrap narrow"><div className="case-label">{label(block.beat, number)}</div><div><h2>{block.heading}</h2>{block.intro && <p>{block.intro}</p>}<div className="insight-cards">{block.insights.map(item => <article key={item.label}><span>{item.label}</span><strong>{item.statement}</strong></article>)}</div></div></section>
}
export function DecisionBlock({ block, number }: { block: BlockOf<'decision'>; number: string }) {
  ...
  return <section className="decision section-wrap"><span className="kicker">{label(block.beat, number)}</span><h2>{block.heading}</h2>{block.figure && <img src={block.figure.src} alt={block.figure.alt} width={block.figure.width} height={block.figure.height} />}<div className="decision-grid">{items.map(item => <div key={item.label}><span>{item.label}</span><p>{item.value}</p></div>)}</div></section>
}
export function OutcomeBlock({ block, number }: { block: BlockOf<'outcome'>; number: string }) {
  return <section className="outcome section-wrap"><span className="kicker">{label(block.beat, number)}</span><h2>{block.heading}</h2><p>{block.body}</p>{block.metrics && <div className="metric-grid">{block.metrics.map(metric => <div key={metric.label}><strong>{metric.value}</strong><span>{metric.label}</span><small>Source: {metric.source}</small></div>)}</div>}{block.unverifiedNote && <small>{block.unverifiedNote}</small>}</section>
}
export function ReflectionBlock({ block }: { block: BlockOf<'reflection'> }) {
  return <section className="reflection section-wrap"><div><span className="kicker">{beatLabels[block.beat]}</span><h2>{block.heading}</h2></div><p>{block.body}</p></section>
}
...
export function CaseStudyBody({ blocks }: CaseStudyBodyProps) {
  return <>{blocks.map((block, index) => {
    ...
    switch (block.kind) {
      case 'prose': return <ProseBlock block={block} number={number} key={block.id} />
      case 'figure': return <FigureBlock block={block} key={block.id} />
      case 'quote': return <QuoteBlock block={block} key={block.id} />
      case 'insight': return <InsightBlock block={block} number={number} key={block.id} />
      case 'decision': return <DecisionBlock block={block} number={number} key={block.id} />
      case 'outcome': return <OutcomeBlock block={block} number={number} key={block.id} />
